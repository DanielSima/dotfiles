...
#original author: Moritz Wirger, https://gist.github.com/enwi
#removed HDD
...
# Basic configuration
c_FAN = 18					# gpio pin the fan is connected to
c_FAN_TACHO	= 23			# gpio pin the fan tachometer is connected to
c_MIN_TEMPERATURE = 55		# temperature in degrees c when fan should turn on
...
if __name__ == '__main__':
	...
	while killer.thread_dont_terminate:	# main loop
		...
		#print "CPU: %d Fan: %d RPM: %d" % (cpu, desired_fan, rpm)	# debug information
		...
		if (cpu < c_TEMPERATURE_OFFSET):	# check if temperature is low enough to turn off
			...
			if ((cpu >= last_cpu) and desired_fan < 100):	# check if temperature is rising or staying the same
				...
				desired_fan -= 5
			print "CPU: %dFan: %d RPM: %d" % (cpu, desired_fan, rpm)	# debug information
